...
(() => {
  ...
  async function sparqlElem({ elem, query }) {
    ...
    try {
      ...
      if (view.type === "inline") {
        ...
        elem.innerHTML = "Executing query:";
        let sparqlElem = document.createElement("pre");
        sparqlElem.innerText = query;
        sparqlElem.style.fontSize = "70%";
        sparqlElem.style.margin = "2ex";
        elem.appendChild(sparqlElem);
        ...
        iframe.src =
          "https://query.wikidata.org/embed.html#" + encodeURIComponent(query);
      }
    } catch (e) {
      elem.innerHTML = "<h1>Error executing SPARQL-query</h1>";
      console.log(e);
      throw e;
    }
  }
  ...
  async function sparqlToIframe(sparql, element, filename) {
    ...
    infoLink.style.float = "right";
    infoLink.style.fontSize = "smaller";
    infoLink.innerHTML = `<a href="https://github.com/fnielsen/scholia/blob/master/scholia/app/templates/${filename}">${filename.replace(
      "_",
      ": "
    )}</a>`;
    elem.parentElement.after(infoLink);
  ...
  async function loadDependencies() {
    ...
    for (const name in visualisations) {
      visualisations[name].resultBrowser =
